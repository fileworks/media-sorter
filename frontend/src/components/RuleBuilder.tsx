...
export function RuleBuilderInline({
  ...
  const renderCollection = (kind: RuleKind, collection: (TagRule | RouteRule)[]) =>
    collection.map((rule, index) => (
      <div key={rule.id} className="rounded-panel border border-border px-3 py-2">
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-panel border border-info/40 bg-info/10 p-3 text-xs text-info">
      ...
      {config.rules_enabled && (
        <>
          ...
          {showForm ? (
            <div className="space-y-3 rounded-panel border border-info/40 bg-info/10 p-4">
